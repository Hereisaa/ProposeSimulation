function [ Model, centr_node ] = CHRNselection_2( Model, locAlive, noOfk, centr, netArch )
    ...
    for i =1:nodeArch.init_numNodes
        if ~isempty(nodeArch.node(i).CID)
            ...
            [ calSCH, calSRN ] = calSelection( E_res, E_avg(nCID,1), E_max(nCID,1), E_min(nCID,1), d_Centr, d_toCentr(nCID,1), d_BS, d_toBS(nCID,1),...
                                    maxToCentr(nCID,1), minToCentr(nCID,1), maxToBS(nCID,1), minToBS(nCID,1), 'proposed2');
    ...
    for i = locAlive
        ...
        % Layer != 0 
            ...
            if  strcmp(nodeArch.node(i).type,'C') 
                        ...
                        for j = M
                           for k =1:size(clusterNode.no,2)
                               if clusterNode.no(k) == j
                                   indexM = [indexM, clusterNode.no(k)];
                                   locM = [locM, [clusterNode.loc(k,1);clusterNode.loc(k,2)]];
                               end
                           end
                        end
                        ...
                        toCH = calDistance(nodeArch.node(i).x, nodeArch.node(i).y, nodeArch.node(id).x, nodeArch.node(id).y);
