...
%     centr = [];
    ...
    if recluster == true
        ...
        if ~isempty(Temp_xy)
            %%% Clustering Phase
            [ p_clusterModel, noOfk, cluster, centr ] = Clustering( p_clusterModel, notLayerZero, Temp_xy, Temp_index, T1, T2, 0 );
            recluster = false;
...
%% PROPOSED*2 ROUND LOOP
p_clusterModel.nodeArch   = init_nodeArch; % node's arch for Proposed
p_clusterModel.netArch   = netArch;
p_clusterModel.clusterFun = 'proposed';
p_clusterModel.nodeArch.init_numNodes = numNodes;
p_clusterModel.nodeArch.numAlive = numNodes; % number of Alive nodes
par_proposed = struct;
recluster    = true;
reselect     = true;
T1 = 87;
...
%     centr = [];
    ...
    if recluster == true
        ...
        if ~isempty(Temp_xy)
            %%% Clustering Phase
            [ p_clusterModel, noOfk, cluster, centr ] = Clustering( p_clusterModel, notLayerZero, Temp_xy, Temp_index, T1, T2, 1 );
            recluster = false;
...
% %     tl_clusterModel = dissEnergyGH(tl_clusterModel, roundArch, netArch);
%     
