...
for i = 1:p_nodeArch.numNode
    ...
    if( dist > d_th )
        ...
        Temp_xy(1, notLayerZero + 1) = p_nodeArch.node(i).x;
        Temp_xy(2, notLayerZero + 1) = p_nodeArch.node(i).y;
...
for i = 1:length(locAlive)
    locAlive_loc(i,1) = p_nodeArch.node(locAlive(i)).x;
    locAlive_loc(i,2) = p_nodeArch.node(locAlive(i)).y;
end
...
for i = 1:p_nodeArch.numNode
    ...
    % CM -> CH
        ...
        for j =1:noOfk
            if(p_nodeArch.node(i).CID == centr_node(j).CID) % in the same cluster
    ...
    if p_clusterModel.nodeArch.numDead > 0 % nodeArch.numNode
        ...
        for i = 1:init_nodeArch.numNode
            if ( p_clusterModel.nodeArch.node(i).type == 'D' )  
                fprintf('[Proposed] FND node = %f,%f. id =%d\n', p_clusterModel.nodeArch.node(i).x, p_clusterModel.nodeArch.node(i).y, i);
            end
        end
        break;
    end
